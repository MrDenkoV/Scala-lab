...
object ExprEval {
  def simplify(e: Expr): Expr = e match {
    ...
    case BinOp("+", Number(0), Number(right))   => Number(right)
    case BinOp("*", Number(1), Number(right))   => Number(right)
    //...
    case _ => e
  }
...
object Appl471 {
  def main(args: Array[String]) = {
    ...
    println(simplify(BinOp("*", Number(1), Number(10))))
    println(evaluate(BinOp("+", 
                     BinOp("+", Number(1.5), Number(5.5)),
                     Number(3))))
    println(evaluate(BinOp("-", Number(1.5), Number(5.5))))
    println(evaluate(BinOp("*", Number(1.5), Number(5.5))))
  }
}